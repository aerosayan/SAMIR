...
def collectPointsFromSplice(xi,collect,lexd):
	...
	collectionMatrix = [[]]
	# collection matrix cound : as lex indices are collected the row no. will increase
	cmCount = 0;
	while i<n: #{ Iterate through lexd
		I = i
		i = i+1 # update lexd iterator
		lex = lexd[I]
		# Go through the lexd from xi to n
		...
		while j<m: #{ Iterate through collectOrder list
			...
			if(c.uid == sv.POINT.uid):
				...
				print('Collecting LINE')
				collectionMatrix[cmCount] = collectSplineObjects(collectOrderPos[J],sv.LINE,lexd)
				...
				print('Collecting SPLINE')
				collectionMatrix[cmCount] = collectSplineObjects(collectOrderPos[J],sv.SPLINE,lexd)
				collectionMatrix.append([]) # create space for next collection
				cmCount += 1 # Increase collection matrix counter
				print('mark 10')
				...
				print('Could not collect the requseted lexeme')
				print(collect)
				exit()
		#}
...
def verifyCFG(xi,cfg,collect,lexd):
	...
	while i<n: #{ 
		...
		if(c.uid == sv.COLLECT.uid):
			...
			# Do collection
			collectPointsFromSplice(I,collect,lexd)
			# skip collected entitites
			print('exit mark2')
			exit()
		elif(c.uid == sv.COLLECT_INDICES.uid):
